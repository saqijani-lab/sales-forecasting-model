...
if data_source == "Upload my own sales data":
    ...
    if uploaded_file is not None:
        ...
        if len(historical) < 3:
            ...
            st.stop()
        elif len(historical) < 24:
            st.warning(
                "This dataset has fewer than 24 months of history, which "
                "isn't enough to reliably detect a *yearly* repeating "
                "pattern (e.g. holiday spikes). To avoid inventing a false "
                "seasonal pattern, this forecast will model the overall "
                "**trend only** — treat longer-range predictions with extra "
                "caution."
            )
            use_yearly_seasonality = False
        else:
            use_yearly_seasonality = 5

        # Cap the forecast horizon to a reasonable fraction of available history,
        # since extrapolating far beyond known data becomes unreliable
        max_reasonable_horizon = max(1, len(historical) // 2)

        with st.spinner("Training forecast model on your data..."):
            model = Prophet(yearly_seasonality=use_yearly_seasonality)
...
# ---- User controls ----
slider_max = min(12, max_reasonable_horizon)
if slider_max < 1:
    slider_max = 1
default_months = min(6, slider_max)
months_ahead = st.slider("How many months ahead to forecast?", 1, slider_max, default_months)

if slider_max < 12:
    st.caption(
        f"⚠️ Forecast horizon is capped at {slider_max} month(s) because "
