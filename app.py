...
if data_source == "Upload my own sales data":
    ...
    if uploaded_file is not None:
        ...
        # ---- Optional: known recurring events / holidays specific to this business ----
        with st.expander("➕ Add known recurring events (optional)"):
            st.write(
                "List any recurring events that affect your sales — Black "
                "Friday, back-to-school, a local festival, an annual sale, "
                "etc. Add **one row per occurrence** (e.g. Friday "
                "2023 and Friday 2024 as separate rows) so the model "
                "can learn the pattern. Include past *and* upcoming dates "
                "if you know them."
            )
            events_input = st.data_editor(
                pd.DataFrame({
                    "Event Name": pd.Series(dtype="str"),
                    "Date": pd.Series(dtype="str"),
                    "Days Before": pd.Series(dtype="int"),
                    "Days After": pd.Series(dtype="int"),
                }),
                num_rows="dynamic",
        ...
        holidays_df = None
        if not events_input.empty:
            valid_events = events_input.dropna(subset=["Event Name", "Date"])
            valid_events = valid_events[valid_events["Event Name"].astype(str).str.strip() != ""]
            if not valid_events.empty:
                parsed_dates = pd.to_datetime(valid_events["Date"], errors='coerce')
                valid_events = valid_events.assign(ds=parsed_dates).dropna(subset=["ds"])
                if not valid_events.empty:
                    holidays_df = pd.DataFrame({
                        "holiday": valid_events["Event Name"].astype(str),
                        "ds": valid_events["ds"],
                        "lower_window": -valid_events["Days Before"].fillna(0).astype(int),
                        "upper_window": valid_events["Days After"].fillna(0).astype(int),
                    })
                    st.success(f"Using {len(holidays_df)} custom event date(s) in this forecast.")
...
with st.expander("ℹ️ About this model & its limitations"):
    ...
    st.write(
        "**Custom events:** if known recurring events (Friday, "
        "back-to-school, local festivals, annual sales) were provided, "
        "the model learns a specific effect for each one instead of "
        "relying only on general trend/seasonality — this usually "
        "improves accuracy around those dates."
    )
